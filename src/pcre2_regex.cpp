...
Pcre2Regex::Pcre2Regex(const std::string& pattern) : regex_(nullptr), match_data_(nullptr), is_valid_(false) {
  int error_code;
  ...
  if (regex_ == nullptr) {
    ...
    std::cerr << "PCRE2 compilation failed at offset " << error_offset << ": " << error_buffer << std::endl;
    return;
  }
...
std::vector<Match> Pcre2Regex::findAll(const std::string& text) const {
  ...
  while (offset < subject_length) {
    ...
    result.push_back({
      text.substr(match_start, match_length),
      match_start
    });
    ...
    // If the match was empty, move forward by one character to avoid infinite loop
